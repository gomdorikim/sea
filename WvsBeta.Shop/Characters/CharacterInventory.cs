...
namespace WvsBeta.Shop {
	...
	public class CharacterInventory {
		...
		public void LoadInventory() {
			Server.Instance.CharacterDatabase.RunQuery("SELECT mesos, equip_slots, use_slots, setup_slots, etc_slots, cash_slots FROM characters WHERE id = " + mCharacter.mID.ToString());
			...
			Server.Instance.CharacterDatabase.RunQuery("SELECT mapindex, mapid FROM teleport_rock_locations WHERE charid = " + mCharacter.mID.ToString());
			...
			Server.Instance.CharacterDatabase.RunQuery("SELECT items.*, pets.* FROM items LEFT JOIN pets ON items.cashid = pets.id WHERE charid = " + mCharacter.mID.ToString());
			data = Server.Instance.CharacterDatabase.Reader;
			...
			foreach (Item it in mItems[4]) {
				if (it != null && Constants.isPet(it.ItemID)) mCharacter.mPets.LoadPet(it);
			}
		}
		...
		public void AddItem(byte inventory, short slot, Item item, bool isLoading) {
			...
			try {
				...
				if (slot < 0) {
					...
					mCharacter.mPrimaryStats.AddEquipStarts(slot, item, isLoading);
				}
				else {
				...
				Console.WriteLine("[{0}] {1}", mCharacter.mName, ex.ToString());
			}
		}
		...
		public void SetItem(byte Inventory, short Slot, Item item) {
			...
			if (Slot < 0) {
				...
				mCharacter.mPrimaryStats.AddEquipStarts(Slot, item, false);
			}
			else {
		...
		public Item GetCashItem(long cashid, byte Inventory) {
			Item temp = null;
			for (short i = 1; i <= mMaxSlots[Inventory - 1]; i++) { // Slot 1 - 24, not 0 - 23
				temp = GetItem(Inventory, i);
				if (temp != null && temp.CashId == cashid) return temp;
			}
			return null;
		}
		...
		public short AddItem2(Item item) {
		...
		public void AddNewItem(int id, short amount) // Only normal items!
		{
			if (!DataProvider.Items.ContainsKey(id) && !DataProvider.Equips.ContainsKey(id)) {
				return;
			}
			...
			try {
				...
				for (int i = 0; i < 5; i++) 
                {
					...
					foreach (Item item in mItems[i]) 
                    {
                        ...
                        {
							PacketHelper.AddItemData(packet, item, item.InventorySlot, false, true);
						}
					}
		...
		public Item CreateNewPet(int itemid, string petname = "") {
			...
			item.Amount = 1;
			item.ItemID = itemid;
			item.CashId = Server.Instance.GetNewCashSerial();
			AddItem2(item);


			Server.Instance.CharacterDatabase.RunQuery("INSERT INTO pets (id, `name`, expiration) VALUES (" + item.CashId.ToString() + ", '" + MySqlHelper.EscapeString(petname) + "', " + Tools.GetTicksWithAddition(new TimeSpan(90, 0, 0, 0)).ToString() + ")");

			mCharacter.mPets.LoadPet(item);
			return item;
		}
	}
}
