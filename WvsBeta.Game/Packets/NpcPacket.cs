...
namespace WvsBeta.Game
{
    public class NpcPacket
    {
                ...
                case 0x02: // recharge
                    {
                        ...
                        byte inv = 2;
                        Item item = chr.Inventory.GetItem(inv, itemslot);
                        if (item == null || !Constants.isRechargeable(item.ItemID) || !DataProvider.NPCs[chr.ShopNPCID].Shop.Any((a) => { return a.ID == item.ItemID;}) )
                        {
                            chr.mPlayer.Socket.Disconnect();
                            return;
                        }
                        ...
                        {
                            ShopItemData sid = DataProvider.NPCs[chr.ShopNPCID].Shop.First((a) => { return a.ID == item.ItemID; });

                            ItemData data = DataProvider.Items[item.ItemID];
                            int Cash = 1;
                            short maxslot = (short)(data.MaxSlot + chr.Skills.GetRechargeableBonus());

                            Cash = (int)(-1 * sid.UnitRechargeRate * maxslot);
        ...
        public static void SendNPCChatTextNote(Character chr, int NpcID, string Text, bool back, bool next)
        {
            Packet pw = new Packet(0xAB);
            pw.WriteByte(0x04);
            pw.WriteInt(NpcID);
            pw.WriteByte(0);
            pw.WriteString(Text);
