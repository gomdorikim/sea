    ...
    public class Mob : MovableLife
    {
        ...
        public bool GiveDamage(Character fucker, int amount, bool pWasPoison = false)
        {
            if (fucker != null)
            {
                if (amount > 99999 && !fucker.Admin)
                {
                    ...
                    fucker.SetHP(1);
                    fucker.SetMP(0);
                    MessagePacket.SendMegaphoneMessage("", string.Format("{0} : I was just p0wned by the Anti-Hack system. Damage hax!", fucker.Name), true);
        ...
        public bool CheckDead(Pos killPos = null)
        {
            ...
            if (maxDmgChar != null)
            {
                ...
                {
                    maxDmgChar.AddEXP((double)(EXP * maxDmgChar.Inventory.GetExtraEXPRate() * Server.Instance.RateMobEXP * Map.MAP_PREMIUM_EXP)); //Premium Maps, also known as Internet Cafe maps
                }
                else
                {
                    maxDmgChar.AddEXP((uint)(EXP * maxDmgChar.Inventory.GetExtraEXPRate() * Server.Instance.RateMobEXP));
                }

                if (maxDmgChar.PartyID != -1)
                {
                    int PartyLevel = 0;
                    int PartyCount = 0;
                    double AvgPartyLevel = 0.0;

                    foreach (Character pMember in DataProvider.Maps[maxDmgChar.Map].Characters)
                    {
                        if (pMember.PartyID == maxDmgChar.PartyID)
                        {
                            PartyCount++;
                            if (maxDmgChar.PrimaryStats.Level - pMember.PrimaryStats.Level <= 10)
                            {
                                //MessagePacket.SendNoticeMap("heh", maxDmgChar.Map);
                                PartyLevel += pMember.PrimaryStats.Level;
                            }
                        }

                    }
                    AvgPartyLevel = PartyLevel / PartyCount;
                    ...
                    foreach (Character pMember in DataProvider.Maps[maxDmgChar.Map].Characters)
                    {
                        if (pMember.PartyID == maxDmgChar.PartyID)
                        {
                            if (maxDmgChar.PrimaryStats.Level - pMember.PrimaryStats.Level <= 10)
                            {
                                //MessagePacket.SendNoticeMap("lolawt : " + PartyLevel.ToString(), maxDmgChar.Map);
                                if (maxDmgChar.ID != pMember.ID)
                                {
                                    //MessagePacket.SendNoticeMap("party exp : " + Constants.PartyEXP(AvgPartyLevel, PartyCount, (EXP * pMember.Inventory.GetExtraEXPRate() * Server.Instance.RateMobEXP), maxDmgChar.PrimaryStats.Level, pMember.PrimaryStats.Level).ToString(), pMember.Map);
                                    pMember.AddPartyEXP((int)Constants.PartyEXP(AvgPartyLevel, PartyCount, (EXP * pMember.Inventory.GetExtraEXPRate() * Server.Instance.RateMobEXP), maxDmgChar.PrimaryStats.Level, pMember.PrimaryStats.Level));
                                    //pMember.AddPartyEXP((uint)(EXP * maxDmgChar.Inventory.GetExtraEXPRate() * Server.Instance.RateMobEXP));
                                }
                            }
                        }
                ...
                DropPacket.HandleDrops(maxDmgChar, MapID, Constants.getDropName(MobID, true), SpawnID, Position, Data.ExplosiveReward, Data.PublicReward, false);

                if (maxDmgChar.Quests.mQuests != null)
                {
                    foreach (KeyValuePair<int, QuestData> kvp in maxDmgChar.Quests.mQuests)
                    {
                        if (!kvp.Value.Complete)
                        {
                            if (kvp.Value.Mobs.Count >= 1)
                            {
                                foreach (KeyValuePair<int, QuestMobData> kvp2 in kvp.Value.Mobs)
                                {
                                    if (kvp2.Value.MobID == this.MobID && kvp2.Value.Killed < kvp2.Value.Needed)
                                    {
                                        Console.WriteLine("needed!!!");
                                        kvp2.Value.Killed++;
                                        QuestPacket.SendQuestMobUpdate(maxDmgChar, (short)kvp.Value.QuestID);
                                    }
                                }
                            }
                        }
            ...
            if (md.Revive != null)
            {
                ...
                {
                    SendMobReviveSpawn(MapID, mobid, Position, Foothold);
                }
            }
        ...
        public void SendMobReviveSpawn(int MapID, int mobid, Pos Position, short Foothold)
        {
