...
namespace WvsBeta.Game
{
    ...
    public partial class Character : CharacterBase
    {
        ...
        public void CharactersendPacket(byte[] buffer)
        {
            if (mPlayer != null && mPlayer.Socket != null)
            {
                mPlayer.Socket.SendData(buffer);
            }
        }
        ...
        public bool Load()
        {
            ...
            else
            {
                ...
                {
                    Server.Instance.CharacterDatabase.RunQuery("SELECT serial FROM character_wishlist WHERE charid = " + ID.ToString());
