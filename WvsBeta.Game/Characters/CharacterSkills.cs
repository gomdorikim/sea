...
namespace WvsBeta.Game
{
    public class CharacterSkills
    {
        ...
        public void DoSkillCost(int skillid, byte level, bool IsParty = false)
        {
            ...
            if (skillid == (int)Constants.DragonKnight.Skills.DragonRoar)
            {
                int lefthp = (int)(double)((double)mCharacter.PrimaryStats.MaxHP * ((double)data.XValue / 100.0d));
                mCharacter.DamageHP((short)lefthp);
            }
            else if (skillid == (int)Constants.Spearman.Skills.HyperBody)
            {
                int leftmp = (int)(double)((double)mCharacter.PrimaryStats.MaxMP * ((double)data.YValue / 100.0d));
                int lefthp = (int)(double)((double)mCharacter.PrimaryStats.MaxHP * ((double)data.XValue / 100.0d));
                if (!mCharacter.PrimaryStats.HasBuff((int)Constants.Spearman.Skills.HyperBody))
                {
                    if (mCharacter.Buffs.GetActiveSkillLevel(skillid) != 0) return; // Buffed
                   
                   // mCharacter.PrimaryStats.BuffBonuses.MaxHP = (short)lefthp;
                    mCharacter.ModifyMaxHP((short)lefthp, false, false);

                    //mCharacter.PrimaryStats.BuffBonuses.MaxMP = (short)lefthp;
                    mCharacter.ModifyMaxMP((short)leftmp, true);
                    //mCharacter.SetMP(mCharacter.PrimaryStats.GetMaxMP(false));
                    //mCharacter.SetHP(mCharacter.PrimaryStats.GetMaxHP(false));

                }
                mCharacter.Buffs.AddBuff(skillid, level, lefthp, leftmp);
                //MessagePacket.SendNotice(mCharacter.PrimaryStats.MaxHP.ToString(), mCharacter);
            }
            ...
            if (!IsParty)
            {
                ...
                if (cash > 0)
                {
                    ...
                    short realAmount = (short)mCharacter.Randomizer.ValueBetween(min, max);
        ...
        public void UseMeleeAttack(int skillid)
        {
            ...
            if (skillid == (int)Constants.FPMage.Skills.PoisonMist)
            {
                DataProvider.Maps[mCharacter.Map].CreateMist(mCharacter, mCharacter.ID, skillid, level, sld.BuffTime, sld.LTX, sld.LTY, sld.RBX, sld.RBY);
            }
        }
        ...
        public void UseRangedAttack(int skillid, short pos)
        {
            ...
            if (mCharacter.PrimaryStats.HasBuff((int)BuffValueTypes.ShadowPartner))
            {
                hits *= 2;
            }

            if (pos > 0 && !mCharacter.PrimaryStats.HasBuff((int)BuffValueTypes.SoulArrow))
            {
