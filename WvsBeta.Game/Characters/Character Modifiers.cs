...
namespace WvsBeta.Game
{
    public partial class Character
    {
        ...
        {
            PrimaryStats.HP = (short)(amount > PrimaryStats.HP ? 0 : PrimaryStats.HP - amount);
            CharacterStatsPacket.SendStatChange(this, (uint)CharacterStatsPacket.Constants.Hp, PrimaryStats.HP);
            ModifiedHP();
        }
        ...
        public void ModifyMaxMP(short value, bool sendpacket = true)
        {
        ...
        public void ModifyMaxHP(short value, bool sendpacket = true, bool UpdateBuffMHP = true)
        {
            //MessagePacket.SendNotice("used modify max HP!", this);
        ...
        public void SetLevel(byte value)
        {
            ...
            if (Quests.mQuests.Count > 0)
            {
                foreach (KeyValuePair<int, QuestData> kvp in Quests.mQuests)
                {
                    //MessagePacket.SendNotice(kvp.Value.Mobs.Count.ToString(), this);
                    if (kvp.Value.Mobs.Count > 0 && !kvp.Value.Complete)
                    {
                        QuestPacket.SendQuestMobUpdate(this, (short)kvp.Value.QuestID);
                    }
                }
            }
        }
        ...
        public void AddPartyEXP(double value) { AddEXP((uint)value, true); }

        public void AddEXP(double value) { AddEXP((uint)value); }

        public void AddEXP(uint value, bool Party = false)
        {
            ...
            if (amnt > Constants.GetLevelEXP(PrimaryStats.Level))
            {
                ...
                short mpgain = 0;
                short hpgain = 0;
                short job = (short)((PrimaryStats.Job / 100) - (PrimaryStats.Job % 100));
                short x = 1;
        ...
        public void AddMesos(int value, bool isSelf = false)
        {
            ...
            InventoryPacket.NoChange(this); //:S
        }
        ...
        public void ChangeMap(int mapid)
        {
            ...
            if (mParty != null)
            {
                foreach (Character member in mParty.Members)
                {
                    mParty.ReceivePartyMemberHP(member);
                    mParty.UpdatePartyMemberHP(member);
                }
            }
             * **/
            if (UsingTimer == true)
            {
                MapPacket.MapTimer(this, Events.KerningCity.CurrentTick);
            }
            /**
            if (DataProvider.Maps[mapid].Reactors.Count > 0)
            {
                foreach (Reactor reactor in DataProvider.Maps[mapid].Reactors)
                {
                    ReactorPacket.SpawnReactor(this, reactor);
                }
        ...
        public void ChangeMap(int mapid, byte mappos)
        {
            ...
            /**
            if (DataProvider.Maps[portal.ToMapID].Reactors.Count > 0)
            {
                foreach (Reactor reactor in DataProvider.Maps[portal.ToMapID].Reactors)
                {
                    ReactorPacket.SpawnReactor(this, reactor);
                }
            ...
            if (this.Door != null)
            {
                //MessagePacket.SendMegaphoneMessage("X : " + portal.X + " Y : " + portal.Y);
                //MapPacket.SpawnDoor(this, (DataProvider.Maps[this.Map].Town) ? true : false, portal.X, portal.Y);
        ...
        {
            return (short)Server.Instance.Randomizer.ValueBetween(1, Constants.BaseHp.Variation);
        }
        public short GetRandomMPOnVariation()
        {
            return (short)Server.Instance.Randomizer.ValueBetween(1, Constants.BaseMp.Variation);
        }
        ...
        {
            Skin = id;
            CharacterStatsPacket.SendStatChange(this, (byte)CharacterStatsPacket.Constants.Skin, id);
        }
